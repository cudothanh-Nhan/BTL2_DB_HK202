...
namespace E_BookStore.DAO
{
    ...
    class DetailDAO
    {
        ...
        public List<Review> getAllReview(int productId)
        {
            ...
            var conn = new MySqlConnection(connString);
            conn.Open();
            string sqlStatement = "call GetAllReview(" + productId + ");";
            var cmd = new MySqlCommand(sqlStatement, conn);
            var reader = cmd.ExecuteReader();
            ...
            while (reader.Read())
            {
                ...
                review.Date = reader.GetString(findIndex(columnName, "Date"));
                review.Username = reader.GetString(findIndex(columnName, "Cus_Username"));
                if (!reader.IsDBNull(findIndex(columnName, "Image_URL")))
